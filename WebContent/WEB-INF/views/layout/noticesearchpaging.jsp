...
<ul class="pagination">
	...
	<%	} else {%>
		<li>
			<a href="/board/noticelist/list?select=<%=request.getAttribute("select") %>&data=<%=request.getAttribute("data") %>&curPage=<%=paging.getStartPage() - paging.getPageCount() %>">
				&laquo;
			</a>
		</li>
	<%	} %>
	...
	<%	if( paging.getCurPage() > 1 ) { %>	
	<li>
		<a href="<%=request.getContextPath() %>/board/noticelist/list?select=<%=request.getAttribute("select") %>&data=<%=request.getAttribute("data") %>&curPage=<%=paging.getCurPage()-1 %>">
			&lt; 
		</a>
	</li>
	<%	} %>
